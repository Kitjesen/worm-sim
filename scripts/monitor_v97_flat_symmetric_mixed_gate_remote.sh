...
RUN_DIR="${RUN_DIR:-runs/worm_v6_ppo_flat_random_v97_server_symmetric_mixed_gate_from_v96progress_np2}"
...
export WORM_V6_ENABLE_MIXED_PLANAR_HARDCASE_GATE="${WORM_V6_ENABLE_MIXED_PLANAR_HARDCASE_GATE:-1}"
export WORM_V6_ENABLE_MIXED_PLANAR_CONTINUOUS_GATE="${WORM_V6_ENABLE_MIXED_PLANAR_CONTINUOUS_GATE:-1}"
export WORM_V6_ENABLE_MIXED_PLANAR_AUTHORITY_REBALANCE="${WORM_V6_ENABLE_MIXED_PLANAR_AUTHORITY_REBALANCE:-1}"
...
log() {
  printf '[%s] %s\n' "$(date -Is)" "$*" | tee -a "server_logs/v97_flat_symmetric_mixed_gate_monitor_${TODAY}.log"
}
...
for model_name in final_model progress_best_model best_model; do
  scan_one "v97_${model_name}_nominal" "$model_name" nominal &
  pids+=("$!")
  scan_one "v97_${model_name}_robust" "$model_name" robust \
...
summary_args=()
for item in \
  v97_final_model_nominal \
  v97_final_model_robust \
  v97_progress_best_model_nominal \
  v97_progress_best_model_robust \
  v97_best_model_nominal \
  v97_best_model_robust; do
...
if [[ "${#summary_args[@]}" -gt 0 ]]; then
  "$PY" src/v6/summarize_scan_acceptance_v6.py \
    "${summary_args[@]}" \
    --title "Worm V6 V97 flat symmetric mixed-gate acceptance" \
    --out-json "${SCAN_DIR}/v97_flat_symmetric_mixed_gate_summary.json" \
    --out-csv "${SCAN_DIR}/v97_flat_symmetric_mixed_gate_summary.csv" \
    --out-md "${SCAN_DIR}/v97_flat_symmetric_mixed_gate_summary.md"
else
