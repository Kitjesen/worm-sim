...
def run_pipe_crawl(bend_radius=0.20, channel_width=0.056,
    ...
    """
    # ---- Cable model with spherical joints (Riddle-style) ----
    # Key upgrade: cable_constraint='connect' → ball joints at cable-plate
    # junctions (3DOF position only), allowing cables to rotate freely.
    # Combined with reduced bend_stiff, the body becomes passively flexible
    # enough to conform to pipe walls during turning.
    params = {
        'num_segments': 5,
        'no_cables': 0,              # REAL cable model
        'cable_constraint': 'connect',  # spherical joints (not rigid weld)
        'cable_weld_solref': '0.005 1', # best turning performance
        'bend_stiff': 5e7,           # 2x softer than steel — best visual/turning tradeoff
        'twist_stiff': 1e6,          # proportionally reduced
    ...
    # ---- Collision groups: cables contact ground only (not walls) ----
    # Cable geoms: contype=4, conaffinity=4
    # Ground plane: contype=7, conaffinity=7
    # This prevents cable-wall jamming while keeping cable-ground friction
    xml_str = re.sub(
    ...
    # ---- Inject visual cable capsules (rigid to plate bodies) ----
    # Real cable geoms shift to one side during bending (connect allows
    # free rotation at endpoints). Visual cables are fixed to plate frames
    # and always display correctly.
    xml_str = inject_visual_cables(xml_str, P)
...
"""
    ...
    # ---- Yaw joint qpos indices (for rendering orientation fix) ----
    # <connect> constraints don't transmit orientation, so plate yaw
    # drifts during bending.  We compute the correct yaw from neighbor
    # positions and set it temporarily before each render frame.
    yaw_qpos_idx = []
    for p in range(num_plates):
        jid = mujoco.mj_name2id(m, mujoco.mjtObj.mjOBJ_JOINT, f"p{p}_yaw")
        yaw_qpos_idx.append(m.jnt_qposadr[jid])

    # ---- Hide real cable geoms, show visual cables instead ----
    # Real cables shift to one side during bending (connect allows free
    # endpoint rotation). Visual cables are rigid to plate frames = correct.
    n_hidden = 0
    for gi in range(m.ngeom):
        geom_name = mujoco.mj_id2name(m, mujoco.mjtObj.mjOBJ_GEOM, gi) or ""
        # Real cable geoms: prefixed "c{seg}s{strip}" (e.g., c0s0G0, c0s0G1, ...)
        # Also ring ball geoms: "ringball{seg}s{strip}"
        if re.match(r'^c\d+s\d+', geom_name) or geom_name.startswith("ringball"):
            m.geom_rgba[gi, 3] = 0.0  # fully transparent
            n_hidden += 1
    print(f"Hidden {n_hidden} real cable geoms (visual cables replace them)")

    # ---- Color plates: head=red, tail=blue ----
    for gi in range(m.ngeom):
        body_id = m.geom_bodyid[gi]
        geom_name = mujoco.mj_id2name(m, mujoco.mjtObj.mjOBJ_GEOM, gi) or ""
        # Skip wall/ceiling/visual cables
        if geom_name.startswith(("wall_", "vc", "rb", "ceil")):
            continue
        if body_id == head_id:
    ...
    for step in range(total_steps):
        ...
        if record_video and step % frame_interval == 0:
            # --- Fix plate orientations for rendering ---
            # <connect> constraints don't transmit rotation, so plates
            # keep their original yaw in the bend.  Compute correct yaw
            # from neighbor positions, set temporarily, render, restore.
            qpos_backup = d.qpos.copy()
            for p in range(num_plates):
                if p == 0:
                    dx = d.xpos[pids[1], 0] - d.xpos[pids[0], 0]
                    dy = d.xpos[pids[1], 1] - d.xpos[pids[0], 1]
                elif p == num_plates - 1:
                    dx = d.xpos[pids[p], 0] - d.xpos[pids[p-1], 0]
                    dy = d.xpos[pids[p], 1] - d.xpos[pids[p-1], 1]
                else:
                    dx = d.xpos[pids[p+1], 0] - d.xpos[pids[p-1], 0]
                    dy = d.xpos[pids[p+1], 1] - d.xpos[pids[p-1], 1]
                d.qpos[yaw_qpos_idx[p]] = math.atan2(dx, dy)
            mujoco.mj_forward(m, d)
    ...
    if record_video and frames:
        ...
        vid_path = os.path.join(vid_dir, f"pipe_crawl_R{int(bend_radius*1000)}.mp4")
