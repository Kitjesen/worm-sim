...
seg_length = 0.065
plate_radius = 0.022
plate_geom_r = 0.014       # visual plate (smaller than cable anchor)
...
for seg in range(num_segments):
    tname = f"rt{seg}"
    ring_tendons_xml += f'    <spatial name="{tname}" width="0.0015" rgba="0.85 0.2 0.2 1">\n'
    for si in range(num_strips):
...
def reshape_to_flat_strips(scene, plate_y_positions=None):
    ...
    for i in range(scene.ngeom):
        ...
        # Hide segments near plate positions (avoid visual clipping)
        if plate_y_positions is not None:
            radial_dist = math.sqrt(pos[0]**2 + (pos[2] - z_center)**2)
            if radial_dist < plate_geom_r + 0.003:
                hide_seg = False
...
for step in range(total_steps):
    ...
    if len(frames) < t * framerate:
        ...
        plate_y_pos = [d.xpos[pids[p], 1] for p in range(num_plates)]
