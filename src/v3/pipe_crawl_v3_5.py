"""
Pipe Crawl V3.5 — Dynamic Visual Steel Strips (No Cable Physics)
================================================================
...
Physics uses no-cable plate model (fast, no strip distortion).
Visual steel strips are computed dynamically at RENDER TIME from
actual plate positions — strips always follow the plates perfectly,
even through tight bends.

Changes from V3.4:
  - no_cables=1: no physical cable composite bodies
  - Dynamic visual strips: computed from plate positions each frame
  - 8 strips × 5 segments × 6 sub-capsules = 240 visual capsules + 40 ring balls
  - Strips follow plate rotation/position exactly — no distortion in bends
  - Gait: axial muscles only (no ring muscles)

Usage:
    python pipe_crawl_v3_5.py --video             # record video
    python pipe_crawl_v3_5.py --radius 200        # bend radius (mm)
    python pipe_crawl_v3_5.py --quick             # short 10s test run
"""
import mujoco
import numpy as np
import math
...
# =====================================================================
#  Dynamic visual strip injection into MjvScene
# =====================================================================
...
def capsule_geom_params(p0, p1, radius):
    ...
    Returns: (pos, mat_flat, size) for mjv_initGeom.
    """
    mid = (p0 + p1) * 0.5
    diff = p1 - p0
    ...
    # Capsule Z-axis aligned with diff
    z_ax = diff / length
...
def inject_visual_strips(scene, d, plate_ids, num_segments):
    ...
    Args:
        scene: MjvScene from renderer.scene
        d: MjData (current state)
        plate_ids: list of plate body IDs [plate0, plate1, ..., plate5]
        num_segments: number of segments (5)
    """
    ...
    for seg in range(num_segments):
        ...
        for si, angle in enumerate(STRIP_ANGLES):
            ...
            # Compute world-frame vertices along the strip
            verts = []
            for k in range(NUM_VERTS):
                t = k / (NUM_VERTS - 1)

                # Interpolated body-center position
                center = (1.0 - t) * pos_i + t * pos_j

                # Interpolated rotation matrix (linear interp + re-orthonormalize)
                ...
                # Local offset (in body frame: X=lateral, Y=forward, Z=up)
...
def run_pipe_crawl(bend_radius=0.20, channel_width=0.056,
    ...
    V3.5: no_cables=1 + dynamic visual strips at render time.
    """
    params = {
        'num_segments': 5,
        'no_cables': 1,              # NO cable physics — fast, no distortion
        'plate_constraint': 'connect',  # inter-plate connection
        'cable_weld_solref': '0.005 1',
        'plate_stiff_x': 0.0,       # free lateral — for passive turning
        'plate_stiff_y': 0.0,
        'plate_stiff_yaw': 0.0,
        'plate_stiff_z': 50.0,      # keep plates at correct height
        'plate_damp_y': 10.0,       # smooth forward motion
        'plate_damp_z': 3.0,        # prevent z oscillation
...
"""
    ...
    print(f"Mode: no_cables=1 + dynamic visual strips ({NUM_STRIPS}×{num_segments}×{NUM_VERTS-1} capsules/frame)")
    ...
    for step in range(total_steps):
        ...
        # ---- Gait control (axial muscles only, no ring) ----
        d.ctrl[:] = 0
        states = get_states(t)
        for seg in range(num_segments):
            s = states[seg]
        ...
        if record_video and step % frame_interval == 0:
            ...
            # Inject dynamic visual strips into the scene
    ...
    print(f"\n{'='*60}")
    print(f"  Pipe Crawl Results (V3.5)")
    ...
    if record_video and frames:
        ...
        vid_path = os.path.join(vid_dir, f"pipe_crawl_R{int(bend_radius*1000)}_v3_5.mp4")
        mediapy.write_video(vid_path, frames, fps=fps)
        print(f"\nVideo saved: {vid_path} ({len(frames)} frames)")

        # Extract key frames including t=32s
        n_frames = len(frames)
        for i, pct in enumerate([0.10, 0.40, 0.65, 0.90]):
            idx = min(int(n_frames * pct), n_frames - 1)
            frame_path = os.path.join(vid_dir, f"pipe_frame_v3_5_{i}.png")
            import cv2
            cv2.imwrite(frame_path, cv2.cvtColor(frames[idx], cv2.COLOR_RGB2BGR))
            print(f"  Frame {idx}/{n_frames} -> {frame_path}")

        # Extra: t=32s specifically
        t32_idx = min(int(32 * fps), n_frames - 1)
        t32_path = os.path.join(vid_dir, "pipe_v3_5_t32s.png")
        import cv2
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Pipe crawl V3.5 — dynamic visual strips")
