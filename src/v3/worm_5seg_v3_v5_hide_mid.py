...
seg_midpoints = [seg * seg_length + seg_length / 2 for seg in range(num_segments)]
...
def reshape_to_flat_strips(scene):
    ...
    for i in range(scene.ngeom):
        ...
        # Hide segments near segment midpoints (circumferential bulge zone)
        if any(abs(pos[1] - my) < 0.012 for my in seg_midpoints):
            g.rgba[3] = 0.0
            continue
