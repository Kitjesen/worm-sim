"""
5-Segment Worm Robot V3 — Circular Locomotion (Asymmetric Wave Phase)
=====================================================================
Uses phase offset between left and right axial muscles to create
curved trajectory. Instead of physically bending stiff cable segments,
the differential wave timing creates asymmetric propulsion that
naturally curves the path.

Method: left muscles contract with a phase DELAY relative to right muscles.
This means the right side pushes forward slightly earlier than the left,
causing the body to curve leftward (CCW).
...
bend_stiff = 5e6            # reduced for lateral flexibility
twist_stiff = 2e6
...
plate_stiff_yaw = 5.0      # low → free yaw rotation for circular locomotion
...
# --- Inter-plate connect constraints (position only, free yaw for steering) ---
plate_welds_xml = ""
for seg in range(num_segments):
    plate_welds_xml += (
...
steer_muscle_force = 200  # high force for wide diagonal tendons
steer_tendons_xml = ""
steer_muscles_xml = ""
for seg in range(num_segments):
    p_start = seg
...
# ======================== Sinusoidal Wave + Constant Steering ========================
wave_period = 1500.0  # ms

# Steering level: how hard to push leftward during extension phase (0-1)
steer_level = 0.5   # constant left-turn activation
...
frames = []
dt = m.opt.timestep
sim_time = 25.0  # seconds (longer for circular arc visibility)
...
for step in range(total_steps):
    ...
    # Control — Sinusoidal Wave + Constant Left Steering
    d.ctrl[:] = 0
    if t >= settle_time:
        t_active_ms = (t - settle_time) * 1000.0
        steer_base = num_axial + num_ring
        for seg in range(num_segments):
            # Axial muscles: same wave on all 4
            ax_act = peristaltic_axial(t_active_ms, seg)
            for mi in range(4):
                d.ctrl[seg * 4 + mi] = ax_act
            # Ring muscles: anti-phase
            d.ctrl[num_axial + seg] = peristaltic_ring(t_active_ms, seg)
            # Constant left-turn steering on all segments
            d.ctrl[steer_base + seg * 2] = steer_level
    ...
    if (step + 1) % 10000 == 0:
        ...
        print(f"  t={t:.1f}s | head x={hx*1000:+.1f} y={hy*1000:+.1f}mm | heading={hdg:+.1f}° | Z={z_range*1000:.1f}mm")
...
displacement = math.sqrt(head_x**2 + (head_y - 0)**2)  # from origin
...
try:
    ...
    ax1.set_title(f"V3 Circular Gait — XY Trajectory\n"
                  f"Gait {{n₂=2, n₃=0, n₁=1 | nP=1}}, heading={heading:.1f}°")
    ax1.legend()
