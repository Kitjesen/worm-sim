# Worm V6 Hardware Trial: sand

- Terrain: `sand`
- Policy bundle: `record/v6/deploy_bundles/sand_random`
- Mode label: `random`
- Recommended gait_blend from scan: `1.000`
- Required video reference in CSV: `record/v6/videos/sand_random_hardware_demo.mp4`
- Controller JSONL example: `record/v6/hardware/field_trials/current/sand/sand_random_controller_stream_example.jsonl`
- Controller JSONL schema: `record/v6/hardware/field_trials/current/sand/sand_random_controller_stream_schema.json`
...
```powershell
python src\v3\hardware_policy_runtime_v6.py --bundle-dir record\v6\deploy_bundles\sand_random --input-jsonl - --output-jsonl - --policy-log-csv record\v6\hardware\sand_random_YYYYMMDD.csv --terrain sand --mode random --gait-blend 1.000 --video-file record/v6/videos/sand_random_hardware_demo.mp4 --max-action-delta 0.2
```
...
```powershell
python src\v3\import_hardware_trial_v6.py --terrain sand --mode random --raw-csv record\v6\hardware\field_trials\current\sand\sand_random_raw.csv --video-file record/v6/videos/sand_random_hardware_demo.mp4 --gait-blend 1.000 --date YYYYMMDD
```

### run deploy policy on raw sensors and write formal audit CSV

```powershell
python src\v3\hardware_policy_runtime_v6.py --bundle-dir record\v6\deploy_bundles\sand_random --input-raw-csv record\v6\hardware\field_trials\current\sand\sand_random_raw.csv --output-csv record\v6\hardware\field_trials\current\sand\sand_random_actions.csv --policy-log-csv record\v6\hardware\sand_random_YYYYMMDD.csv --terrain sand --mode random --gait-blend 1.000 --video-file record/v6/videos/sand_random_hardware_demo.mp4 --max-action-delta 0.2 --validate-policy-log --require-video --expected-terrain sand --min-rows 5 --min-duration 0.1
```

### validate policy CSV for the paper audit

```powershell
python src\v3\validate_hardware_log_v6.py --input record\v6\hardware\sand_random_YYYYMMDD.csv --expected-terrain sand --require-video --min-rows 5 --min-duration 0.1
```

### optional: replay deploy bundle on logged observations

```powershell
python src\v3\deploy_policy_v6.py replay --bundle-dir record\v6\deploy_bundles\sand_random --input-csv record\v6\hardware\sand_random_YYYYMMDD.csv --output-csv record\v6\hardware\field_trials\current\sand\sand_random_actions.csv
```
