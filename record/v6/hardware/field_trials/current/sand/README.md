...
```powershell
python src\v6\hardware_policy_runtime_v6.py --bundle-dir record\v6\deploy_bundles\sand_random --input-jsonl - --output-jsonl - --policy-log-csv record\v6\hardware\sand_random_YYYYMMDD.csv --terrain sand --mode random --gait-blend 0.500 --video-file record/v6/videos/sand_random_hardware_demo.mp4 --max-action-delta 0.2
```
...
```powershell
python src\v6\import_hardware_trial_v6.py --terrain sand --mode random --raw-csv record\v6\hardware\field_trials\current\sand\sand_random_raw.csv --video-file record/v6/videos/sand_random_hardware_demo.mp4 --gait-blend 0.500 --date YYYYMMDD
```

### run deploy policy on raw sensors and write formal audit CSV

```powershell
python src\v6\hardware_policy_runtime_v6.py --bundle-dir record\v6\deploy_bundles\sand_random --input-raw-csv record\v6\hardware\field_trials\current\sand\sand_random_raw.csv --output-csv record\v6\hardware\field_trials\current\sand\sand_random_actions.csv --policy-log-csv record\v6\hardware\sand_random_YYYYMMDD.csv --terrain sand --mode random --gait-blend 0.500 --video-file record/v6/videos/sand_random_hardware_demo.mp4 --max-action-delta 0.2 --validate-policy-log --require-video --expected-terrain sand --min-rows 5 --min-duration 0.1
```
