...
```powershell
python src\v6\hardware_policy_runtime_v6.py --bundle-dir record\v6\deploy_bundles\flat_random --input-jsonl - --output-jsonl - --policy-log-csv record\v6\hardware\flat_random_YYYYMMDD.csv --terrain flat --mode random --gait-blend 0.500 --video-file record/v6/videos/flat_random_hardware_demo.mp4 --max-action-delta 0.2
```
...
```powershell
python src\v6\import_hardware_trial_v6.py --terrain flat --mode random --raw-csv record\v6\hardware\field_trials\current\flat\flat_random_raw.csv --video-file record/v6/videos/flat_random_hardware_demo.mp4 --gait-blend 0.500 --date YYYYMMDD
```
